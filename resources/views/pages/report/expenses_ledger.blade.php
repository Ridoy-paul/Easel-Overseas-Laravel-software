...
<div class="content">
    <div class="block block-rounded">
        <div class="row p-2">
            <div class="col-md-3"><h4 class="">Expenses Ledger</h4></div>
            <div class="col-md-3">
                <select name="" class="form-control" id="exp_category">
                    <option value="all">All Expenses</option>
                    @foreach($categories as $category)
                    <option value="{{$category->id}}">{{$category->title}} [{{$category->expense_type}}]</option>
                    @endforeach
                </select>
            </div>
...
<script type="text/javascript">
    ...
    $('#exp_category').change(function() {
        $('#date_range_start_date').val('');
        $('#date_range_end_date').val('');
        $('#single_day_date').val('');
        $('#day_book_body').html('');
        success('Expenses Category is Changed, Next select Date!');
    });
    ...
    function day_book(first_date, last_date) {
        ...
        $.ajax({
            type: 'get',
            url: '/report/expenses-ledger_data',
            data: {
